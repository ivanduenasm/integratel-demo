...
export default function VoiceCall() {
  ...
  const handleStartCall = async () => {
    ...
    try {
      ...
      await retellWebClient.startCall({ accessToken: access_token, sampleRate: 24000 });
    } catch (e) {
      console.error(e);
      setIsStarting(false);
    }
  ...
  const handleStopCall = () => retellWebClient?.stopCall();

  return (
    <div className="relative flex flex-col items-center justify-center p-8 sm:p-10 rounded-3xl w-full overflow-hidden min-h-[320px]">

      {/* ── Sound Wave Animation (background layer) ── */}
      <SoundWave isActive={isCalling} isAgentTalking={isAgentTalking} />

      {/* ── All UI is above the wave ── */}
      <div className="relative z-10 flex flex-col items-center w-full">

        {/* Avatar / Status Icon */}
        <div className={`relative w-24 h-24 rounded-full mb-6 flex items-center justify-center transition-all duration-700 ${
          isCalling
            ? "bg-gradient-to-tr from-[#5CE1E6] to-[#8C52FF] shadow-[0_0_40px_rgba(140,82,255,0.7)]"
            : "bg-white/5 border border-white/10"
        }`}>
          {isCalling && (
            <div className="absolute inset-0 rounded-full bg-[#8C52FF] animate-ping opacity-20" />
          )}
          {isCalling ? (
            <div className={`transition-all duration-300 ${isAgentTalking ? "scale-110" : "scale-100"}`}>
              <Sparkles className={`w-10 h-10 text-white ${isAgentTalking ? "animate-pulse" : ""}`} />
            </div>
          ) : (
            <Phone className="w-9 h-9 text-gray-500" />
          )}
        </div>

        {/* Status text */}
        <h3 className="text-3xl font-bold mb-3 text-white tracking-tight text-center">
          {isCalling
            ? isAgentTalking
              ? "Agente Hablando..."
              : "Agente Escuchando..."
        ...
        <p className="text-center text-gray-400 mb-8 text-base max-w-[280px]">
          {isCalling
